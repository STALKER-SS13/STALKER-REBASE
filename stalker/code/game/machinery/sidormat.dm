...
GLOBAL_LIST_INIT(global_sidormat_list, list()) // delete when list is finished
/*
...
/datum/data/stalker_equipment/stalker_pda/GetCost(var/obj/item/stalker_pda/PDA)
	var/cost_ = src.sale_price
	if(PDA.owner && (PDA.owner.stat == DEAD))
		var/rank_ = clamp(PDA.profile.rating, ROOKIE, ZONE_LEGEND)
		cost_ += GetCostBasedOnReputation(PDA.profile.reputation)
		cost_ += rank_*2
	return cost_
...
/obj/machinery/stalker/sidormat/ui_interact(mob/living/carbon/human/H)
	...
	var/datum/stalker_profile/profile = PDA.profile

	if(!profile || !PDA.owner)
	...
	///////////////////////////////////////////////////////////АНГЛИЙСКИЙ СИДОРОМАТ///////////////////////////////////////////////////////////////////////
		...
		for(var/L in GLOB.global_sidormat_list)
			...
			for(var/datum/data/stalker_equipment/prize in GLOB.global_sidormat_list[L])
				if((sk.fields["faction_s"] == prize.faction && ((profile.faction in special_factions) || (switches & SHOW_FACTION_EQUIPMENT))) || prize.faction == "Everyone")
...
/obj/machinery/stalker/sidormat/Topic(href, href_list)
	...
	var/datum/data/record/profile = find_record("sid", H.sid, GLOB.data_core.stalkers)
	var/obj/item/stalker_pda/PDA = H.wear_id
